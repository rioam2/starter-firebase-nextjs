...
(async function main() {
    ...
    for (const [envName, envConfig] of Object.entries(environments)) {
        if (await confirm(`Setup a ${FORMAT.yellow}${envName}${FORMAT.reset} environment?`)) {
            ...
            const setupProject =
                !config[projectVar] ||
                (await confirm(`Replace existing ${envName} project, ${config[projectVar]}?`));
            if (setupProject) {
            ...
            const setupWebapp =
                !config[webappVar] ||
                (await confirm(`Replace existing ${envName} webapp for ${projectId}?`));
            if (setupWebapp) {
    ...
    const deployPrompt = config[deployEnv]
        ? 'Generate a new Firebase deployment key?'
        : 'Generate a Firebase deployment key?';
    ...
    (await confirm(`Store in ${FORMAT.yellow}.env${FORMAT.reset} (plaintext) for local use?`)) &&
        saveEnvToFile(config);
    (await confirm(`Store in ${FORMAT.yellow}.travis.yml${FORMAT.reset} (encrypted) for CI/CD?`)) &&
        saveEnvToTravis(config);
})();
...
async function selectWebapp(parent, label) {
    const webapps = ((await listFirebaseProjectApps(parent)) || []).filter(
